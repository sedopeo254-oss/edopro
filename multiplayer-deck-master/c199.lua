...
function s.candidatemask(e,tp,eg,ep,ev,re,r,rp,chain)
    ...
    local mask=0
    for p=0,MP_MAX_PLAYERS-1 do
        if p~=owner and (active&(1<<p))~=0 and Duel.IsLogicalPlayerActive(p)
    ...
    if tc:IsMonster() then
        ...
        if te then
            ...
            if (tpe&TYPE_FIELD)~=0 then
                loc=LOCATION_FZONE
                local fc=is_battle_royal() and Duel.GetFieldCard(tp,LOCATION_FZONE,0) or Duel.GetFieldCard(1-tp,LOCATION_FZONE,0)
                if Duel.IsDuelType(DUEL_1_FIELD) and not is_battle_royal() then
