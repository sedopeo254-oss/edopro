...
local function get_logical_monsters(logical)
    ...
    return g or Group.CreateGroup()
end
local function get_candidate_player_mask()
    local active=Duel.GetActiveLogicalPlayerMask and Duel.GetActiveLogicalPlayerMask() or 0
    local mask=0
    for p=0,MP_MAX_PLAYERS-1 do
        if (active&(1<<p))~=0 and Duel.IsLogicalPlayerActive(p) then
            local g=get_logical_monsters(p)
            if g:GetCount()>0 then mask=mask|(1<<p) end
        end
    end
    return mask
end
...
local function get_code_482_monster(tp)
    if is_battle_royal() and Duel.GetLogicalPlayer then
...
function s.condition(e,tp,eg,ep,ev,re,r,rp)
    ...
    if is_battle_royal() and Duel.GetLogicalPlayer then
        ...
        local attacker=att:GetLogicalControler()
        return owner~=nil and attacker~=nil and attacker~=owner
    end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
    ...
    if is_battle_royal() and Duel.GetPlayerFieldGroup then
        ...
        local g=get_logical_monsters(logical):Filter(s.filter,nil)
        Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
        local sg=g:Select(tp,1,1,nil)
        e:SetLabel(val)
        if #sg>0 then Duel.SetTargetCard(sg) end
        return
    end
    if chk==0 then
...
function s.operation(e,tp,eg,ep,ev,re,r,rp)
    ...
    if tc and tc:IsControler(1-tp) and tc:IsFaceup() and tc:IsMonster()
