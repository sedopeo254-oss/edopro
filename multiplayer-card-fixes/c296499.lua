...
--Anime multiplayer fix: declared Type cannot attack, including an attack already in progress.
local s,id=GetID()
function s.initial_effect(c)
	--Activate / declare a Type
	...
	c:RegisterEffect(e1)
	--Monsters of the declared Type cannot attack at all.
...
function s.atktarget(e,c)
	...
	local rc=ae and ae:GetLabel() or 0
	return rc~=0 and (c:GetRace()&rc)~=0
end
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	local a=Duel.GetAttacker()
	local rc=e:GetLabel()
	if a and rc~=0 and (a:GetRace()&rc)~=0 then
		Duel.NegateAttack()
	end
end
