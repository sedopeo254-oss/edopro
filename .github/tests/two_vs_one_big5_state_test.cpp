...
int main() {
    ...
    assert(big5.logical_player(1, 1) == MultiplayerState::NO_PLAYER);

    assert(big5.prompt_player_of(0) == 2);
    assert(big5.prompt_player_of(1) == 3);
    assert(big5.prompt_player_of(2) == 1);

    assert(big5.current_player() == 2); // Big Five first
    assert(big5.advance_turn() == 0);  // Joey
    assert(big5.advance_turn() == 1);  // Yugi
    assert(big5.advance_turn() == 2);  // Big Five
    ...
    // Protected modes must retain their exact original topology/order.
    MultiplayerState br;
