...
function stateOf(m){const s=m.state||m.snapshot||{};return{turn:num(s.turn??m.turn),phase:s.phaseName||PHASE[s.phase??m.phase]||s.phase??m.phase??null,lp:Array.isArray(s.lp)?s.lp:[s.lp0??null,s.lp1??null],hand:Array.isArray(s.hand)?s.hand:[s.hand0??null,s.hand1??null]}}
...
class BridgeReceiver extends EventEmitter{
  ...
  stop(){if(!this.server)return;for(const s of this.server._connections?[]:[])try{s.destroy()}catch{};try{this.server.close()}catch{}this.server=null;this.connected=false}
